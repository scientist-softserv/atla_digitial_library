# OVERRIDE Hyrax 2.5 to use background job for work reindexing on collections
module Hyrax
  module Collections
    module NestedCollectionPersistenceService
      ...
      # @note Adding the member_of_collections method doesn't trigger reindexing
      #       of the child so we have to do it manually.
      ...
      def self.persist_nested_collection_for(parent:, child:)
        ...
        ReindexChildrenJob.perform_later(child.id)
      end

      # @note Removing the member_of_collections method doesn't trigger reindexing
      #       of the child so we have to do it manually.
      #       However it doesn't save and reindex the parent,
      #       as it does when a parent is added!!
